...
export class SidenavItemComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    ...
    if (storedItem) {
      this.router.navigateByUrl(storedItem);
    }
  ...
  onSelect(route: string): void {
    ...
    if (storedItem === route) {
      ...
      const previousItem = document.querySelector('.vex-sidenav-item--active');
      if (previousItem) {
        previousItem.classList.remove('vex-sidenav-item--active');
      }
